package com.rs.leanbacknative.utils;

public class Constants {
    public static final String FOCUS_DIRECTION_UP = "up";
    public static final String FOCUS_DIRECTION_DOWN = "down";
    public static final String FOCUS_DIRECTION_LEFT = "left";
    public static final String FOCUS_DIRECTION_RIGHT = "right";
    public static final String FOCUSED_CARD_ALIGNMENT_LEFT = "left";
    public static final String FOCUSED_CARD_ALIGNMENT_CENTER = "center";
    public static final String FOCUSED_CARD_ALIGNMENT_LEANBACK = "leanback";
    public static final String CARD_SHARE_ROUND = "round";
    public static final String CARD_SHARE_SQUARE = "square";
    public static final String COMMAND_REQUEST_FOCUS = "request-focus";
    public static final String COMMAND_REQUEST_FOCUS_BY_ID = "requestFocusById";
    public static final String EVENT_ON_PRESS = "onPress";
    public static final String EVENT_ON_FOCUS = "onFocus";
    public static final String EVENT_ON_DATA_IDS_READY = "onDataIdsReady";
    public static final String IMAGE_TRANSFORMATION_NO_TRANSFORMATION = "noTransformation";
    public static final String IMAGE_TRANSFORMATION_CENTER_CROP = "centerCrop";

    public static final int DEFAULT_CARD_WIDTH = 313;
    public static final int DEFAULT_CARD_HEIGHT = 176;
}
